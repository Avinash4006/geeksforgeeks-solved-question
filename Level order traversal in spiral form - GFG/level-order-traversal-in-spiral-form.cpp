...
vector<int> findSpiral(Node *root)
{
    ...
    vector<pair<Node*, int>> vp;
    q.push(make_pair(root, 0));
    vp.push_back(make_pair(root, 0));
    ...
    while (!q.empty()) {
        ...
        if (ptr->left) {
            ...
            vp.push_back(make_pair(ptr->left, level + 1));
        }
        if (ptr->right) {
            q.push(make_pair(ptr->right, level + 1));
            vp.push_back(make_pair(ptr->right, level + 1));
        }
    }
    ...
    for (int i = 0; i < n; i++) {
        int j = i;
        if (vp[i].second % 2 == 0) {
            while (j < n && vp[j].second == vp[i].second)
                j++;
